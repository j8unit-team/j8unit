package org.j8unit.repository.javax.imageio.stream;

import javax.imageio.stream.ImageOutputStreamImpl;
import org.j8unit.runners.J8Unit4;
import org.junit.AssumptionViolatedException;
import org.junit.runner.RunWith;

/**
 * Specific JUnit test class to proof the instance relevant aspects of type {@link ImageOutputStreamImpl} (by simply
 * reusing the J8Unit test interface {@link org.j8unit.repository.javax.imageio.stream.ImageOutputStreamImplTests}).
 */
@RunWith(J8Unit4.class)
public class ImageOutputStreamImplTest
implements ImageOutputStreamImplTests<ImageOutputStreamImpl> {

    // J8UNIT-MARKER-[BEGIN]-[INSTANCE]-[javax.imageio.stream.ImageOutputStreamImpl]

    @Override
    public ImageOutputStreamImpl createNewSUT() {
        throw new AssumptionViolatedException("Due to the abstract modifier of this type-under-test [javax.imageio.stream.ImageOutputStreamImpl], j8unit does not support a generic way to provide instances.");
    }

    // J8UNIT-MARKER-[MANUAL]-[INSTANCE]-[javax.imageio.stream.ImageOutputStreamImpl]

    // J8UNIT-MARKER-[END]-[INSTANCE]-[javax.imageio.stream.ImageOutputStreamImpl]

}
