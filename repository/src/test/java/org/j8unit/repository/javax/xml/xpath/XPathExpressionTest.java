package org.j8unit.repository.javax.xml.xpath;

import javax.xml.xpath.XPathExpression;
import org.j8unit.runners.J8Unit4;
import org.junit.AssumptionViolatedException;
import org.junit.runner.RunWith;

/**
 * Specific JUnit test class to proof the instance relevant aspects of type {@link XPathExpression} (by simply reusing
 * the J8Unit test interface {@link org.j8unit.repository.javax.xml.xpath.XPathExpressionTests}).
 */
@RunWith(J8Unit4.class)
public class XPathExpressionTest
implements XPathExpressionTests<XPathExpression> {

    // J8UNIT-MARKER-[BEGIN]-[INSTANCE]-[javax.xml.xpath.XPathExpression]

    @Override
    public XPathExpression createNewSUT() {
        throw new AssumptionViolatedException("Due to the abstract modifier of this type-under-test [javax.xml.xpath.XPathExpression], j8unit does not support a generic way to provide instances.");
    }

    // J8UNIT-MARKER-[MANUAL]-[INSTANCE]-[javax.xml.xpath.XPathExpression]

    // J8UNIT-MARKER-[END]-[INSTANCE]-[javax.xml.xpath.XPathExpression]

}
