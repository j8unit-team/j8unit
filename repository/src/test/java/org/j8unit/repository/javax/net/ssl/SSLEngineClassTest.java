package org.j8unit.repository.javax.net.ssl;

import javax.net.ssl.SSLEngine;
import org.j8unit.runners.J8Unit4;
import org.junit.runner.RunWith;

/**
 * Specific JUnit test class to proof the type relevant aspects of type {@link SSLEngine} (by simply reusing the J8Unit
 * test interface {@link org.j8unit.repository.javax.net.ssl.SSLEngineClassTests}).
 */

@RunWith(J8Unit4.class)
public class SSLEngineClassTest
implements SSLEngineClassTests<SSLEngine> {

    // J8UNIT-MARKER-[BEGIN]-[CLASS]-[javax.net.ssl.SSLEngine]

    @Override
    public Class<SSLEngine> createNewSUT() {
        return SSLEngine.class;
    }

    // J8UNIT-MARKER-[MANUAL]-[CLASS]-[javax.net.ssl.SSLEngine]

    // J8UNIT-MARKER-[END]-[CLASS]-[javax.net.ssl.SSLEngine]

}
