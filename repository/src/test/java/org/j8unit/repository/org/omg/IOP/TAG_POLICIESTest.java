package org.j8unit.repository.org.omg.IOP;

import org.j8unit.runners.J8Unit4;
import org.junit.AssumptionViolatedException;
import org.junit.runner.RunWith;
import org.omg.IOP.TAG_POLICIES;

@RunWith(J8Unit4.class)
public class TAG_POLICIESTest
implements org.j8unit.repository.org.omg.IOP.TAG_POLICIESTests<TAG_POLICIES> {

    // J8UNIT-MARKER-[BEGIN]-[INSTANCE]-[org.omg.IOP.TAG_POLICIES]

    @Override
    public TAG_POLICIES createNewSUT() {
        throw new AssumptionViolatedException("Due to the abstract modifier of this type-under-test [org.omg.IOP.TAG_POLICIES], j8unit does not support a generic way to provide instances.");
    }

    // J8UNIT-MARKER-[MANUAL]-[INSTANCE]-[org.omg.IOP.TAG_POLICIES]

    // J8UNIT-MARKER-[END]-[INSTANCE]-[org.omg.IOP.TAG_POLICIES]

}
